...
class MigrationTests(unittest.TestCase):
    def source(self):
        return json.loads((ROOT / 'Assets/MaterialCanvas/Terrain/Examples/minimal_tint.materialgraph').read_text())
    ...
    def test_connected_tint_preserves_source_nodes_and_rewires_multiply(self):
        ...
        multiply = next(n for n in graph['m_nodes'] if n['Value']['configId'] == migration.config_id('multiply_rgb'))['Key']
        self.assertIn({'m_sourceEndpoint': old['m_sourceEndpoint'], 'm_targetEndpoint': [multiply, {'m_name': 'b'}]}, graph['m_connections'])
        self.assertIn({'m_sourceEndpoint': [multiply, {'m_name': 'result'}], 'm_targetEndpoint': [output_id, {'m_name': 'inBaseColor'}]}, graph['m_connections'])
    ...
    def test_constant_tint_and_existing_base_color_are_both_preserved(self):
        ...
        self.assertEqual(count, 1)
        multiply = next(n for n in result['ClassData']['m_nodes'] if n['Value']['configId'] == migration.config_id('multiply_rgb'))
        self.assertEqual(multiply['Value']['m_inputDataSlots'][1]['Value']['m_value']['Value'], [.2, .4, 1.7])
        self.assertIn({'m_sourceEndpoint': connection['m_sourceEndpoint'], 'm_targetEndpoint': [multiply['Key'], {'m_name': 'a'}]}, result['ClassData']['m_connections'])
        self.assertEqual(migration.migrate(result)[1], 0)

    def test_passthrough_needs_no_migration(self):
        doc = json.loads((ROOT / 'Assets/MaterialCanvas/Terrain/Examples/surface_passthrough.materialgraph').read_text())
