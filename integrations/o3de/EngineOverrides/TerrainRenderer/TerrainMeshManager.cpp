...
namespace Terrain
{
    ...
    void TerrainMeshManager::GatherMeshData(SectorDataRequest request, AZStd::vector<HeightNormalVertex>& meshHeightsNormals, AZ::Aabb& meshAabb, bool& terrainExistsAnywhere)
    {
        ...
        auto perPositionCallback =
            [this, &heights, &positions, &exists, &receivedSamples, querySamplesX, &terrainExistsAnywhere, renderSnapshot, batchQueries](
                size_t xIndex, size_t yIndex, const AzFramework::SurfaceData::SurfacePoint& surfacePoint, bool terrainExists)
        {
            ...
            if (renderSnapshot)
            {
                // Never let collision/query-only mesh cutouts remove terrain vertices.
                // The project terrain shaders perform the visible cut at fragment
                // resolution; this query retains only ordinary authored terrain holes.
                TerrainCompositor::TryGetTerrainRenderGeometryExists(*renderSnapshot, surfacePoint.m_position, renderTerrainExists);
                TerrainCompositor::TryGetTerrainRenderGeometryHeight(*renderSnapshot, surfacePoint.m_position, worldHeight);
            }
        ...
        if (batchQueries)
        {
            ...
            TerrainCompositor::ApplyTerrainRenderGeometry(*renderSnapshot, positions, heights, AZStd::span<bool>(exists.get(), querySamplesCount));
