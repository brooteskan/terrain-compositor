...
| Optional Canvas tint selector | `SystemTickBus` | Owning entity context, RPI scene, or terrain feature processor is not ready | Binding succeeds, stop, or eight attempts expire | Establish scene ownership. Material assets then load/reload through the renderer's asset subscriptions; editing/reactivating the component restarts resolution. |
